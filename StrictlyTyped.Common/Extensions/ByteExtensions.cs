...
namespace StrictlyTyped
{
    public static partial class Extensions
    {
        ...
        public static bool TryAs<TStrict>(this byte? value, [NotNullWhen(true)] out TStrict result, out IReadOnlySet<string> failures)
            where TStrict : struct, IStrictByte<TStrict>
        {
            if (value is null)
            {
        ...
        public static bool TryAsNullable<TStrict>(this byte? value, [NotNullWhen(true)] out TStrict result, out IReadOnlySet<string> failures)
            where TStrict : struct, IStrictByte<TStrict>
        {
            if (value is null)
            {
        ...
        public static bool TryAs<TStrict>(this byte? value, [NotNullWhen(true)] out TStrict result)
            where TStrict : struct, IStrictByte<TStrict> =>
            value.TryAs(out result, out _);


        public static bool TryAsNullable<TStrict>(this byte? value, [NotNullWhen(true)] out TStrict result)
            where TStrict : struct, IStrictByte<TStrict> =>
